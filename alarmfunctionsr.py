...
def UpdateHostThread(function,opcode):
  try:
      _thread.start_new_thread(UpdateHost, (function,opcode, ) )
  except:
...
def UpdateHost(function,opcode):
  try:
    ...
    script_path = Buildscript_path(function,opcode)

    if globals.PrintToScreen: print(("Host Update: "+script_path))

    rt=urllib.request.urlopen(script_path)

    time.sleep(.2)
    temp=rt.read()
    if globals.PrintToScreen: print("Responce is: ",temp," lenght of ",len(temp))
    if len(temp) == 0:
      l = 0
      RecordSet = ['']
    else:
...
def StartSirenThread(Zone):
  try:
      _thread.start_new_thread(Siren, (Zone, ) )
  except:
      print("Error: unable to start thread")
...
def StartChimeThread():
  try:
      _thread.start_new_thread(SoundChime, ())
  except:
      print("Error: unable to start thread")
...
def SendEmailAlertFromRule(ruleid, SensorNumber, photo):
  try:
      _thread.start_new_thread(SendEmailAlertThread, (SensorNumber, ruleid, True, photo, ) )
  except:
      print("Error: unable to start thread")

def SendEmailAlert(SensorNumber):
  try:
      _thread.start_new_thread(SendEmailAlertThread, (SensorNumber,0 , False, False) )
  except:
      print("Error: unable to start thread")
